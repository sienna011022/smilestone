...
class CategoryListAdapter(private val model: CategoryListViewModel, private val context: Context): RecyclerView.Adapter<CategoryListAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(view: View): RecyclerView.ViewHolder(view), View.OnClickListener {
        ...
        override fun onClick(view: View?) {
            ...
            intent.putExtra(PRODUCT_ID, model.product.value?.get(adapterPosition)?.productId)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.title.text =model.product.value?.get(position)?.title
        val time = LocalDateTime.parse(model.product.value?.get(position)?.localDateTime)
        holder.time.text = convertLocalDateTimeToTime(time)
        holder.price.text = priceFormat.format(model.product.value?.get(position)?.price?.toLong()) + "원"
    }

    override fun getItemCount(): Int = model.product.value?.size ?: 0
}
